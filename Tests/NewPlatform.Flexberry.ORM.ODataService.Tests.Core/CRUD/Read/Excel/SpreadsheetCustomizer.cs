namespace NewPlatform.Flexberry.ORM.ODataService.Tests.CRUD.Read.Excel
{
    ...
    //    private static Cell CreateCell(string value, UInt32Value  styleIndex,CellValues cellValues)  // ExcelTypeDefinition excelDataType)
    //    {
    //        var headerCell = new Cell { CellValue = new CellValue(value), StyleIndex = styleIndex, };

    //        if (cellValues != CellValues.Date)
    //        {
    //            headerCell.DataType = new EnumValue<CellValues>(cellValues);
    //        }
    //        return headerCell;
    //    }
    //}
